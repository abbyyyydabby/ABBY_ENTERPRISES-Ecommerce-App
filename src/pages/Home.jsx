    ...
    <Box sx={{ my: 4 }}>
      ...
      <Paper elevation={3} sx={{ borderRadius: 2, overflow: 'hidden', mb: 4 }}>
        <StyledCarousel
          ...
          navButtonsProps={{
            style: { backgroundColor: 'rgba(0,0,0,0.5)', color: '#9974e7ff', borderRadius: 0 },
          }}
          indicatorIconButtonProps={{ style: { padding: 10 } }}
          activeIndicatorIconButtonProps={{ style: { backgroundColor: '#8b4fbfff' } }}
        >
          {bannerImages.map((item, i) => (
            <Box key={i} sx={{ position: 'relative' }}>
              <img src={item.url} alt={item.title} style={{ width: '100%', height: 400, objectFit: 'cover' }} />
              <Box sx={{ position: 'absolute', bottom: 0, left: 0, width: '100%', bgcolor: 'rgba(0,0,0,0.6)', color: '#aa4545ff', p: 3 }}>
